...
## Post-implementation amendments (2026-09-05, after the code review of the diff)

Applied on top of the completed tasks; the spec sections 4.5, 9, 11 and 13 carry the same changes.

- Task 20: `validate_tflite` fails closed when the operator set cannot be enumerated; `feature_spec.json` is emitted next to the headers and promoted; `check_fsm_capacity` refuses FSM parameters beyond the C capacities before `model_meta.h` is written; `promote` removes newly installed trees on rollback; `export card` requires benchmark and DoA reports bound to the current release (`bound_to` hashes in their JSON).
- Task 11: runs record manifest, validation-id and model SHA-256 plus the audit status; `select` uses only current, unmodified runs and needs the full matrix; `final` verifies the selected model hash and recomputes its validation AUC; training refuses an unaudited manifest unless `train.allow_unaudited` is set.
- Task 9 / 5: teacher flag threshold equals the audit threshold (0.1); `audit_status.json` is part of the manifest generation.
- Task 6 / 10: RIRs use ISM order 10 and direct-path-peak normalisation.
- Task 17 / 23: a correlation without a usable peak yields ratio 0 (never valid) in Python and C, with a loud out-of-band golden case; C DoA and feature code use static work buffers and expose int16 entry points; `fsm_init` returns -1 for oversized parameters instead of truncating.
- Tests: silence assertion in `test_sources`, exporter-only assertion is exact, new tests for the fail-closed gates, capacity check, audit requirement, run provenance and out-of-band DoA frames.
- Second review round: `teacher.csv` rows carry the window md5 and `teacher_meta.json` the cache fingerprint, teacher identity and thresholds; `--apply-audit` refuses scores whose fingerprint or md5 differ. `select` validates the exact matrix `{hard_w1: (hard, 1.0), kd_w1: (kd, 1.0), hard_w2: (hard, 2.0), kd_w2: (kd, 2.0)}` and a single validation-id hash. Benchmark and DoA reports bind to the hashes of the files actually evaluated (plus tau / FSM parameters, DoA sweep parameters); the card checks them. `promote_generation` removes backup-less installs on rollback. `train import-candidate` + `train final --run` make fine-tuned candidates deployable through the same verification and calibration. The test-consumption record persists across manifest generations. Makefile targets depend on all headers; `test_doa.c` replays every golden frame through the int16 entry points as well.
- Third review round: `threshold.json` carries the manifest and validation-id hashes and `export model` refuses a deployed model or threshold from another manifest before touching the test split; `export_info.json` records the canonical FSM parameters and the approved DoA sweep (`doa.sim_*` in the config), and the card compares the complete benchmark and DoA bindings; `--apply-audit` validates the teacher identity, classes, thresholds and scored count and records the SHA-256 of `teacher.csv`/`teacher_meta.json` in `audit_status.json`, which training re-checks together with the md5 of every training window before using KD targets or the exclusion list; `manifest_unaudited.csv` keeps the pre-audit assignment so the teacher can be re-run on an audited generation; C gains `doa_tracker_reset_episode` (votes cleared, floor kept, mirrored-episode host test).
- Fourth review round: the test-consumption record stores tau and the SHA-256 of every bundle file; `claim_test_split` refuses a re-calibrated tau or any changed bundle file, and `check_parity` re-decides the gate from the numbers and `PARITY_LIMITS`. `verify_teacher_binding` requires the audited teacher files to exist. The C FSM expires old bursts and hits before appending the current tick (a window equal to the capacity keeps every hit), `gaps`/`vals` are static work buffers, and the host test checks the retained hit and burst sets on every tick of both the release trace and a boundary trace (`fsm_trace_boundary.txt`, confirm + hold + 1 == 64) generated with the golden vectors.
- Fifth review round: `select_config`'s fallback returns the AUC-near-best run with the highest recall (size and hard-over-KD only break exact ties); `promote` drops the release-bound reports (`model_card.md`, `benchmark_nights.md`, `doa_sim.md`, `cross_collection.md`) from the new deliverables tree so nothing derived from the previous release survives a new export; the model card lists the test-consumption record history from `deliverables/test_consumption_resets.log`.
- Sixth review round: cloud payloads carry `in_bed`/`body_motion_level` only when the radar observed them (nothing is invented; the cloud schema defaults apply otherwise); `export reset-test-record --reason` is the only sanctioned reset (ledger entry first, then record and bundles removed) and `claim_test_split` refuses a fresh evaluation while an earlier release or leftover bundle exists without a later ledger entry; the model card prints the ledger; WHL snore files without a qualifying peak are logged as `no_qualifying_peak`.
- Seventh review round: the benchmark binds to the approved benchmark configuration (`benchmark_config`: nights, night length, SNRs, seed) and both the benchmark and the DoA report record the SHA-256 of the markdown rendered from their JSON; `export card` compares the configuration, verifies the markdown hash, and checks that every predictor covers every approved SNR over exactly the approved number of nights.
- Eighth review round: `export card` re-renders each report's markdown from its JSON and refuses any difference; the benchmark must contain exactly the float and int8 predictors with their agreement entry, `n_nights` distinct nights per predictor at approved SNRs, and aggregates equal to the per-night recount; the DoA report also binds to the release's model and TFLite hashes.
- Ninth review round: `check_benchmark_coverage` recomputes every per-SNR aggregate from the per-night records with the generator's own `_aggregate` and refuses any numeric difference (NaN-aware), so an edited aggregate cannot enter the card.
- Tenth review round: `choose_threshold` returns the smallest float32 strictly above the permitted negative (one exact value shared by metrics, parity, benchmark, JSON and `model_meta.h`, which prints it with 9 significant digits); `export card` gains `check_doa_coverage` (one row per approved sweep cell, approved trial count, finite rates in [0, 1]). The threshold change re-calibrated tau (float32-exact), so the same model was re-evaluated once on the test split after a documented reset.
